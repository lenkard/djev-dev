"""Candidate one-token labels; the compiler validates every label in context.

Uppercase single letters precede uppercase pairs. Unsupported pairs are omitted.
A request fails before inference if its actual tokenizer/template cannot represent
all allowed labels as distinct tokens at exactly one answer position.
"""
MAX_LABEL_IDS = 512
